...
public class LinkerUtils {
	...
	public static Collection<Mention> babelfyJSONtoMentions(final String annotatedText, final String inText) {
		...
		try {
			...
			e.printStackTrace();
			// System.err.println("Server message: " + annotatedText);
		}
		return null;
...
//			}
	...
	public static Collection<Mention> magJSONtoMentions(String annotatedText, String inputText, Number defaultScore) {
		/**
		 * [{"disambiguatedURL":"http:\\/\\/dbpedia.org\\/resource\\/Celebrity","offset":13,"namedEntity":"famous
		 * people","start":33},
		 * {"disambiguatedURL":"http:\\/\\/dbpedia.org\\/resource\\/Steve_Jobs","offset":10,"namedEntity":"Steve
		 * Jobs","start":4},
		 * {"disambiguatedURL":"http:\\/\\/dbpedia.org\\/resource\\/Joan_Baez","offset":9,"namedEntity":"Joan
		 * Baez","start":19}]\n
		 */
		final Collection<Mention> ret = new ArrayList<>();
		try {
			...
			for (int i = 0; i < results.length(); ++i) {
				try {
					...
					final MentionMAG mention = new MentionMAG(namedEntity, possAss, start, defaultScore, namedEntity,
							namedEntity);
					ret.add(mention);
				} catch (JSONException exc) {
					exc.printStackTrace();
	 ...
	 * @return
	 */
	public static Collection<Mention> nifToMentions(String annotatedText, Number defaultScore,
			Function<String, String> translate) {
		final Collection<Mention> retMentions = Lists.newArrayList();

		// Caused java.lang.NoSuchMethodError: 'void
		// org.aksw.gerbil.transfer.nif.TurtleNIFDocumentParser.<init>(org.aksw.gerbil.io.nif.impl.TurtleNIFParser)'
		// Therefore replaced with the constructor that doesn't use arguments
		// final TurtleNIFDocumentParser parser = new TurtleNIFDocumentParser(new
		// AgnosTurtleNIFParser());
		...
		try {
			...
			for (Marking m : markings) {
				...
				// TODO Why MentionOpenTapioca instead of some general kind of Mention class?
				final Mention mention = MentionOpenTapioca.create(document.getText(), m); // MentionMarking.create(document.getText(),
																							// m);

				// Added try-catch when getting NullPointerExceptions for FOX, where
				// ProvenenceInfo is provided
				try {
					mention.assignBest();
					mention.getAssignment().setScore(defaultScore);
				} catch (NullPointerException e) {
					getLogger().info("Skipped part of NIF response: " + m.toString());
					continue;
				}
		...
		for (Mention m : retMentions) {
			for (PossibleAssignment possAss : m.getPossibleAssignments()) {
				possAss.setAssignment(translate.apply(possAss.getAssignment()));
			}
		}
		return retMentions;
	}
	...
	/**
	 * Add <tt>&lt;entity&gt;</tt>-Tags to input text with markings (result of
	 * mention detection) to make it ready for candidate generation and entity
	 * disambiguation with MAG <br/>
	 * <br/>
	 * Example: &lt;entity&gt;Steve Jobs&lt;/entity&gt; and &lt;entity&gt;Joan
	 * Baez&lt;/entity&gt; are famous people.
	 * 
...
//                        prev.getStartPosition());
				if (span.getLength() > prev.getLength()) {
					...
					textBuilder.delete(textBuilder.length() - prev.getLength() - mentionTagLength,
							textBuilder.length());
...
//				ret.add(mention.getOffset());
	...
	/**
	 * Convert JSON of a custom API (IP address based component) to a collection of
	 * mention. Read all required fields from the JSON, i.e. surface form and
	 * offset.
	 * 
	 * @param annotatedText
	 * @return
	 */
	@Deprecated
	public static Collection<Mention> JSONToMentions(final String annotatedText) {
		/**
		 * {"doc":{"mentions":[{"sf":"Napoleon","start":0,"end":8}],"text":"Napoleon was
		 * the emperor of the First French Empire."}}
		 */

		final Collection<Mention> mentions = new ArrayList<>();

		// define JSON keys
		final String keyDoc = "doc";
		final String keyMentions = "mentions";
		final String keySurfaceForm = "sf";
		final String keyOffset = "start";

		// parse JSON and get the array with the mentions
		final JSONObject json = new JSONObject(annotatedText);
		final JSONObject doc = json.optJSONObject(keyDoc);
		final JSONArray results = doc.optJSONArray(keyMentions);
		...
		for (int i = 0; i < results.length(); ++i) {
			try {
				// get the mention and read its attributes
				final JSONObject mentionObj = results.getJSONObject(i);
				final Integer offset = mentionObj.getInt(keyOffset);
				final String surfaceForm = mentionObj.getString(keySurfaceForm);

				// create and add the mention
				final Mention mention = new Mention(surfaceForm, offset);
				mentions.add(mention);
	...
	/**
	 * Extract the result document from an API's response JSON.
	 */
	public static AnnotatedDocument apiJSONToDocument(final String json)
			throws JsonMappingException, JsonProcessingException {
		ObjectMapper objectMapper = new ObjectMapper();
		APITaskContainer result = objectMapper.readValue(json, APITaskContainer.class);
		return result.getDocument();
	}

	public static String documentToAPIJSON(AnnotatedDocument document, org.json.simple.JSONObject pipelineConfig,
			String componentId) throws JsonProcessingException {
		final APITaskContainer taskPackage = new APITaskContainer(document, pipelineConfig, componentId);

		// map object to JSON with Jackson
		final ObjectMapper objectMapper = new ObjectMapper();
		final String request = objectMapper.writeValueAsString(taskPackage);
		return request;
	}
