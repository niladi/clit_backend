...
public abstract class AbstractConsolidator implements Consolidator, Loggable {
	 ...
	 */
	public Map<Linker, AnnotatedDocument> executeLinkers(final AnnotatedDocument input) throws InterruptedException {
		final Map<Linker, AnnotatedDocument> mapLinkerResults = new HashMap<>();
		//
		...
		for (Linker linker : linkers) {
			...
			final Future<Integer> future = executor.submit(new Callable<Integer>() {
				...
				public Integer call() throws Exception {
					...
					final AnnotatedDocument result = linker.annotate(input);
					synchronized (mapLinkerResults) {
						mapLinkerResults.put(linker, result);
					}
	 ...
	 * @return
	 */
	public AnnotatedDocument mergeAll(Map<Linker, AnnotatedDocument> mapLinkerMention) {
	...
	public AnnotatedDocument mergeAll(Map<Linker, AnnotatedDocument> mapLinkerMention,
		...
		for (Map.Entry<Linker, AnnotatedDocument> e : mapLinkerMention.entrySet()) {

			final Linker linker = e.getKey();
			final Collection<Mention> otherLinkedMentions = MentionUtils.copyMentions(e.getValue().getMentions());
		...
		final String text = mapLinkerMention.entrySet().iterator().next().getValue().getText();
		return new AnnotatedDocument(text, linkerMentions);
	}
	...
	public Map<String, AnnotatedDocument> mergeByKG(Map<Linker, AnnotatedDocument> mapLinkerMention) {
		final Map<String, AnnotatedDocument> mapMergedResults = new HashMap<>();
		// Go through the map with the linkers and their results
		for (Entry<Linker, AnnotatedDocument> e : mapLinkerMention.entrySet()) {
			...
			// Get the text from the first document (any other would be possible as well)
			final String text = mapLinkerMention.entrySet().iterator().next().getValue().getText();
			if ((linkerMentions = mapMergedResults.get(keyKG).getMentions()) != null) {
				...
				mapMergedResults.put(keyKG, new AnnotatedDocument(text, mergedMentions));
			} else {
				// We don't have this mention/entity combo yet, so just put the modulated
				// version into the results map
				mapMergedResults.put(keyKG, new AnnotatedDocument(text, modulatedMentions));
				// new HashSet<>()
				// e.getValue()
			}
		}
