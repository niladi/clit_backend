...
public class LauncherCoNLLTSVAnnotation {
	...
	public static void main(String[] args) {
		...
		for (int docCounter = startDoc; docCounter < inTexts.size(); ++docCounter) {
			annotate(docCounter, new AnnotatedDocument(inTexts.get(docCounter)), linkers);
		}
	}
	...
	private static void annotate(final int docCounter, final AnnotatedDocument document, final List<Linker> linkers) {
		...
		for (Linker linker : linkers) {
			...
			try {
				...
				annotatedText = annotate(document, linker, 0);
	...
	private static String annotate(AnnotatedDocument document, Linker linker, int attemptCounter) throws InterruptedException {
		String annotatedText = null;
