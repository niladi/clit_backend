...
public class LauncherNIFDatasetAnnotation {
	private static final String outDir = "C:/Users/wf7467/Desktop/Evaluation Datasets/Datasets/entity_linking/output/";
	private static int counter = 0;
	private static BufferedWriter bwLog = null;

	public static final String kore50InPath = "C:\\Users\\wf7467\\Desktop\\Evaluation Datasets\\Datasets\\entity_linking\\"//
			+ "Reuters-128.ttl";
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		// Input TSV file
		final File inFile = new File(kore50InPath);
		System.out.println("Input Data: " + inFile.getAbsolutePath());
		if (!inFile.exists()) {
		...
		try (final BufferedWriter bwLabels = new BufferedWriter(new FileWriter(outDir + "/labels.txt"))) {
			final List<Document> inTexts = parseDocuments(inFile);
			// parseTSV(inFile, startDoc, stopDoc, results);

			// Now that we've collected the input texts, annotate them
			for (int docCounter = startDoc; docCounter < inTexts.size(); ++docCounter) {
				annotate(docCounter, new AnnotatedDocument(inTexts.get(docCounter).getText()), linkers);
//				final Document doc = inTexts.get(docCounter);
//				bwLabels.write(doc.getText());
//				bwLabels.write(cbuf);
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	protected static Model parseNIFModel(Reader reader, Model nifModel) {
	...
	private static List<Document> parseDocuments(File fileKORE50) throws FileNotFoundException {
		return parseDocuments(new BufferedReader(new FileReader(fileKORE50)));
	}

	private static Document parseDocument(Reader reader) throws FileNotFoundException, UnexpectedException {
	...
	private static List<Document> parseDocuments(Reader reader) throws FileNotFoundException {
	...
	public static String makeResultKey(final Integer docCounter, final Integer startOffset, final String surfaceForm) {
		return (docCounter == null ? "no_doc" : docCounter) + "_" + (startOffset == null ? "no_offset" : startOffset)
				+ "_" + (surfaceForm == null ? "no_sf" : surfaceForm);
	}
	...
	private static void annotate(final int docCounter, final AnnotatedDocument document, final List<Linker> linkers) {
		counter++;
		System.out.println("Annotating(Doc#" + docCounter + " (#" + counter + " for this run)");
		System.out.println(document.getText());
		for (Linker linker : linkers) {
			AnnotatedDocument result = null;
			try {
				// retries until it works or failed too often
				result = annotateMentions(document, linker, 0);
			} catch (InterruptedException e) {
				System.err.println("ERROR - Thread.sleep issue");
				e.printStackTrace();
			}
			//outputText(docCounter, result.getText(), linker);
			outputText(docCounter, result.getMentions(), linker);
		}
	...
	private static String annotate(AnnotatedDocument document, Linker linker, int attemptCounter)
			throws InterruptedException {
		String annotatedText = null;
		final int MAX_ATTEMPTS = 20;
		String errorMsg = "", smallErrorMsg = "";
		try {
			annotatedText = linker.annotate(document).toString();
		} catch (Exception ioe) {
			annotatedText = null;
			...
			if (annotatedText == null) {
				if (attemptCounter < MAX_ATTEMPTS) {
					if (errorMsg.contains("response code: 414")) {
						...
						annotatedText = annotate(document, linker, attemptCounter + 1);
					}
				} else {
	...
	private static AnnotatedDocument annotateMentions(AnnotatedDocument input, Linker linker, int attemptCounter)
			throws InterruptedException {
		AnnotatedDocument result = null;
		final int MAX_ATTEMPTS = 20;
		String errorMsg = "", smallErrorMsg = "";
		try {
			result = linker.annotate(input);
		} catch (Exception ioe) {
			result = null;
			final StringWriter errors = new StringWriter();
			ioe.printStackTrace(new PrintWriter(errors));
			...
			if (result == null) {
				if (attemptCounter < MAX_ATTEMPTS) {
					if (errorMsg.contains("response code: 414")) {
						...
						System.out.println("Waiting for [" + waitTime + "ms.]");
						Thread.sleep(waitTime);
						result = annotateMentions(input, linker, attemptCounter + 1);
					}
				} else {
					System.err.println("Too many failed attempts... [" + attemptCounter + "]");
				}
	...
	private static void outputText(int docCounter, Collection<Mention> mentions, Linker linker) {
		final StringBuilder sb = new StringBuilder();
		for (Mention m : mentions) {
			append(sb, docCounter);
			append(sb, linker.getClass().getName());
			append(sb, linker.getWeight().doubleValue());
			append(sb, m.getMention());
			append(sb, m.getOffset());
			append(sb, m.getOffset() + m.getMention().length());
			append(sb, m.getDetectionConfidence());
			append(sb, m.getAssignment() != null ? m.getAssignment().getAssignment() : "none");
			append(sb, m.getAssignment() != null ? m.getAssignment().getScore().doubleValue() : "0.0");
			append(sb, m.getPossibleAssignments().size());
			sb.append(System.getProperty("line.separator"));
		}
		outputText(docCounter, sb.toString(), linker);
	}

	private static void append(StringBuilder sb, Object string) {
		sb.append(string);
		sb.append(";");
	}
